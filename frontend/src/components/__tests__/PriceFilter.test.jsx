...
describe("PriceFilter", () => {
  ...
  it("renders nothing when no hotels have prices", () => {
    const { container } = render(<PriceFilter hotels={hotelsNoPrices} onFilterChange={() => {}} />);
    expect(container.innerHTML).toBe("");
  });

  it("renders nothing when hotels array is empty", () => {
    const { container } = render(<PriceFilter hotels={[]} onFilterChange={() => {}} />);
    expect(container.innerHTML).toBe("");
  });
  ...
  it("updates range when min slider changes", () => {
    ...
    expect(onFilterChange).toHaveBeenLastCalledWith(expect.objectContaining({ min: 200 }));
  });
  ...
  it("updates range when max slider changes", () => {
    ...
    expect(onFilterChange).toHaveBeenLastCalledWith(expect.objectContaining({ max: 500 }));
  });
  ...
  it("prevents min from exceeding max", () => {
    ...
    expect(onFilterChange).toHaveBeenLastCalledWith(expect.objectContaining({ min: 799 }));
  });
  ...
  it("prevents max from going below min", () => {
    ...
    expect(onFilterChange).toHaveBeenLastCalledWith(expect.objectContaining({ max: 301 }));
  });
  ...
  it("resets range when hotels change", () => {
    ...
    expect(onFilterChange).toHaveBeenCalledWith(expect.objectContaining({ min: 100, max: 800 }));
    ...
    expect(onFilterChange).toHaveBeenLastCalledWith(expect.objectContaining({ min: 50, max: 400 }));
  });

  it("handles hotels with the same price", () => {
    const onFilterChange = vi.fn();
    render(<PriceFilter hotels={hotelsSamePrice} onFilterChange={onFilterChange} />);
    expect(onFilterChange).toHaveBeenCalledWith(expect.objectContaining({ min: 200, max: 200 }));
  });
